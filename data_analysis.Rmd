...
## Box Plots
For better understanding, I repeat this step with box plots.
...
## Comparing WRN and ANHD1 Distribution
For comparison, I plot the distribution of dependency scores of a randomly selected KO gene in the Achilles dataset as histograms. The dependency scores of WRN are generally more broadly distributed. When looking at MSI cell lines, this observation becomes much clearer.
```{r}
...
plot_mss + plot_msi
```
I repeat the procedure and additionally calculate the p-values for comparison. The p-value for WRN is very significant, whereas the p-value for ABHD1 is not.
```{r}
...
# Calculating means
I calculate the means of the dependency sores for MSI and MSS cell lines for every gene.
```{r}
...
We calculate the p-values using the Welch-Test of the dependency scores of MSI and MSS cell lines for every gene.
```{r}
...
We calculate the p-values using the Wilcoxon-Test of the dependency scores of MSI and MSS cell lines for every gene.
```{r}
...
## Volcano Plot Using Wilcoxon Test
We plot the mean differences of the dependency scores against the corresponding p-values and get a volcano plot.
```{r}
# transform p-values
...
We calculate again the p-values but with Bayes moderated t-stats using limma [@limma] and plot them against the corresponding log2FoldChanges.
```{r}
