...
fit <- eBayes(fit = fit)
results_bayes <- topTable(fit = fit, n=Inf, coef = 2, sort.by = "none")

p_adj_bayes <- data.frame(matrix(NA, nrow = nrow(results_bayes), ncol = 2))
colnames(p_adj_bayes) <- c('gene','p_adj')
p_adj_bayes$gene <- rownames(results_bayes)
p_adj_bayes$p_adj <- results_bayes$adj.P.Val

# transform p-values
p_adj_bayes_log <- p_adj_bayes
p_adj_bayes_log$p_adj <- NULL
p_adj_bayes_log$log <- -log(p_adj_bayes$p_adj)

# create data structure for mean differences and p-values
data_bayes <- merge(mean_differences, p_adj_bayes_log, by="gene")

# define labeled genes
data_bayes$to_label <- ""
data_bayes$to_label[data_bayes$gene == "WRN"] <- "WRN"

ggplot(data=data_bayes, aes(x=difference,y=log, label=to_label)) +
  geom_point() +
  xlab('logFoldChange') +
  ylab('-log(P_adj)') +
  geom_label_repel()
