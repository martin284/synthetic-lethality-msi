...
# Data Preprocessing
I imported the data provided by the authors as "Supplementary Table 1". The dataset contains MSI/MSS classifications for different cancer cell lines and the corresponding dependency scores for the WRN gene. In addition, I imported the dataset "DepMap_18Q4_data.rds", which contains all dependency scores from the project Achilles and project DRIVE. The weblinks can all be found in @chan.
```{r, warning=FALSE}
...
For quality control, I plotted the dependency scores of WRN which are present in both datasets against each other. The dependency scores of WRN from Achilles significantly correlate to the scores in DRIVE with the Pearson correlation coefficient 0.56. Since the Pearson correlation coefficient is not very high, the analysis should be performed separately to the two datasetsinstead of using the mean values.
```{r}
