...
calculate_means <- function(col,condition) {
  mean <- mean(as.numeric(col[dep_scores_18Q4_achilles_df$CCLE_MSI==condition]))
  return(mean)
}

only_dep_scores <- dep_scores_18Q4_achilles_df[,-c(1,ncol(dep_scores_18Q4_achilles_df))]

mean_msi <- apply(only_dep_scores, 2, calculate_means, condition='MSI')
mean_msi_df <- data.frame(mean_msi)
mean_mss <- apply(only_dep_scores, 2, calculate_means, condition='MSS')
mean_mss_df <- data.frame(mean_mss)
mean_dep_scores <- mean_msi_df
mean_dep_scores$mean_mss <- mean_mss_df$mean_mss
mean_dep_scores <- cbind.data.frame(gene=rownames(mean_dep_scores), mean_dep_scores)
rownames(mean_dep_scores) <- NULL
head(mean_dep_scores)
```

## Calculating P-Values Using Students t-Test
We calculate the p-values of MSI and MSS cell lines for every gene and save the result.
```{r}
calculate_p_values_t <- function(col) {
  test_result <- t.test(col ~ dep_scores_18Q4_achilles_df$CCLE_MSI, alternative="two.sided", var.equal = FALSE)
  return(test_result$p.value)
}

p_values <- data.frame(apply(only_dep_scores, 2, calculate_p_values_t))
p_values <- cbind.data.frame(rownames(p_values),p_values)
rownames(p_values) <- NULL
colnames(p_values) <- c("gene","p_value")

head(p_values)
```

## Volcano Plot Using Students t-Test
...
We calculate the p-values of MSI and MSS cell lines for every gene and save the result.
```{r}
calculate_p_values_wilcox <- function(col) {
  test_result <- wilcox.test(col ~ dep_scores_18Q4_achilles_df$CCLE_MSI, alternative="two.sided", var.equal = FALSE)
  return(test_result$p.value)
}

p_values <- data.frame(apply(only_dep_scores, 2, calculate_p_values_wilcox))
p_values <- cbind.data.frame(rownames(p_values),p_values)
rownames(p_values) <- NULL
colnames(p_values) <- c("gene","p_value")
...
I calculate again the p-values but with Bayes moderated t-stats using limma [@limma].
```{r}
